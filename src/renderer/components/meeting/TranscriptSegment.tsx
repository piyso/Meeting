...
export const TranscriptSegment = memo<TranscriptSegmentProps>(({
  ...
  return (
    <div className={`flex gap-[var(--space-12)] py-[var(--space-8)] min-h-[36px] group transition-colors px-2 -mx-2 rounded ${
      isPinned 
      ...
      <div className="flex items-start gap-[var(--space-8)] w-[140px] shrink-0 mt-[2px]">
        ...
        <div className="flex flex-col">
          ...
          <span className="font-mono text-[10px] text-[var(--color-text-tertiary)] flex items-center gap-1">
            {timestamp}
            {isEdited && <Badge variant="outline" className="scale-[0.8] origin-left border-none bg-[var(--color-bg-glass)] px-1">Edited</Badge>}
          </span>
        </div>
      </div>
  ...
  return prev.id === next.id && prev.isLive === next.isLive && prev.text === next.text && prev.isPinned === next.isPinned
})
